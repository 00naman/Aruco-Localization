    ...
    try:
        ...
        if ids is not None:
            for i in range(len(ids)):
                ...
                # Convert the rotation vector (rvec) to a rotation matrix (rmat)
                ...
                # Create an Odometry message for the ArUco pose
                odometry_msg = Odometry()
                odometry_msg.header.stamp = rospy.Time.now()
                odometry_msg.header.frame_id = "map"
                odometry_msg.child_frame_id = f"aruco_marker_{ids[i]}"
                odometry_msg.pose.pose.position.x = round(float(tvec[0][0][0]), 2)
                odometry_msg.pose.pose.position.y = round(float(tvec[0][0][1]), 2)
                odometry_msg.pose.pose.position.z = round(float(tvec[0][0][2]), 2)
                ...
                rospy.loginfo(f"ArUco marker {ids[i]} found at position (x, y, z): ({tvec[0][0][0]}, {tvec[0][0][1]}, {tvec[0][0][2]})")
        else:
